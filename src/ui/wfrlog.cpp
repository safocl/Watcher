...
btnAdd( " +++ " ), grid(), logEntityArr() {
    ...
    logEntityArr.push_back( Gtk::make_managed<LogEntity>());
    ...
    btnAdd.signal_clicked().connect(sigc::mem_fun(*this, &WFrLog::onBtnClicked));
}
WFrLog::~WFrLog() {}
...
void WFrLog::onBtnClicked(){
    ...
    logEntityArr.push_back(Gtk::make_managed<LogEntity>());
    grid.attach_next_to(*logEntityArr.back(), *logEntityArr[logEntityArr.size()-2], Gtk::POS_BOTTOM,3,1);
